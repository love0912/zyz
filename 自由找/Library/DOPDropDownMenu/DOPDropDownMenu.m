...
- (void)selectWithArray:(NSArray *)tmpArray {
    for (int i = 0; i < tmpArray.count; i ++) {
        [self selectIndexPath:[DOPIndexPath indexPathWithCol:i row:[tmpArray[i] integerValue]] triggerDelegate:NO];
    }
}
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height  {
    ...
    if (self) {
        ...
        if (IS_IPHONE_4_OR_LESS) {
            _tableViewHeight = 280;
        } else if (IS_IPHONE_5) {
            _tableViewHeight = 360;
        } else if (IS_IPHONE_6) {
            _tableViewHeight = 400;
        } else if (IS_IPHONE_6P) {
            _tableViewHeight = 440;
        }
//        _tableViewHeight = IS_IPHONE_4_OR_LESS ? 280 : kTableViewHeight;
        _isClickHaveItemValid = YES;
        if (origin.y !=64) {
            //lefttableView init
            _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x,SCREEN_HEIGHT-44, self.frame.size.width/2, 0) style:UITableViewStylePlain];
            ...
            //righttableView init
            _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + self.frame.size.width/2, SCREEN_HEIGHT-44, self.frame.size.width/2, 0) style:UITableViewStylePlain];
            ...
            _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(origin.x, SCREEN_HEIGHT-44, self.frame.size.width, kButtomImageViewHeight)];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (_directionType==1) {
        ...
        if (show) {
            if (haveItems) {
                _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y, self.frame.size.width/2, 0);
                _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.origin.y, self.frame.size.width/2, 0);
                [self.superview addSubview:_leftTableView];
                [self.superview addSubview:_rightTableView];
            } else {
                _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y, self.frame.size.width, 0);
                _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2, self.origin.y, self.frame.size.width/2, 0);
                [self.superview addSubview:_leftTableView];
                
            }
            _buttomImageView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT-44-kButtomImageViewHeight, self.frame.size.width, kButtomImageViewHeight);
...
//            }
            [UIView animateWithDuration:0.2 animations:^{
                if (self.origin.y==SCREEN_HEIGHT-104-44) {
                     _buttomImageView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT-(43+tableViewHeight+kButtomImageViewHeight+104), self.frame.size.width, kButtomImageViewHeight);
                }else{
                     _buttomImageView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT-(43+tableViewHeight+kButtomImageViewHeight), self.frame.size.width, kButtomImageViewHeight);
                }
                if (haveItems) {
                    
                   
                    if (self.origin.y==SCREEN_HEIGHT-104-44) {
                        _leftTableView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT-(_tableViewHeight+44+104), self.frame.size.width/2, tableViewHeight);
                        
                        _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2,SCREEN_HEIGHT-( _tableViewHeight+44+104), self.frame.size.width/2, tableViewHeight);
                    }else{
                        _leftTableView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT-(_tableViewHeight+44), self.frame.size.width/2, tableViewHeight);
                        
                        _rightTableView.frame = CGRectMake(self.origin.x + self.frame.size.width/2,SCREEN_HEIGHT-( _tableViewHeight+44), self.frame.size.width/2, tableViewHeight);
                    }
                } else {
                    if (self.origin.y==SCREEN_HEIGHT-104-44) {
                        _leftTableView.frame = CGRectMake(self.origin.x,SCREEN_HEIGHT-(tableViewHeight+44+104), self.frame.size.width, tableViewHeight);
                    }else{
                        _leftTableView.frame = CGRectMake(self.origin.x,SCREEN_HEIGHT-(tableViewHeight+44), self.frame.size.width, tableViewHeight);
                    }
            ...
            [UIView animateWithDuration:0.2 animations:^{
                _buttomImageView.frame = CGRectMake(self.origin.x, SCREEN_HEIGHT, self.frame.size.width, kButtomImageViewHeight);
                if (haveItems) {
                    _leftTableView.frame = CGRectMake(self.origin.x, self.origin.y, self.frame.size.width/2, 0);
