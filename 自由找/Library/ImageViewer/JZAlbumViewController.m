...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[PhotoView class]]) {
        ...
        PhotoView *photoV;
        NSObject *obj = [_imgArr objectAtIndex:index];
        if ([obj isKindOfClass:[NSString class]]) {
            photoV = [[PhotoView alloc] initWithFrame:frame withPhotoUrl:(NSString *)obj];
        } else {
            photoV = [[PhotoView alloc] initWithFrame:frame withPhotoImage:(UIImage *)obj];
        }
