...
- (instancetype)initWithTitles:(NSArray *)titles WithVCs:(NSArray *)childVCs WithColorArrays:(NSArray *)colors controllers:(UIViewController *)controllers {
    if (self = [super init]) {
        ...
        self.frame = CGRectMake(0, 64, FUll_VIEW_WIDTH, FUll_CONTENT_HEIGHT);
        myArray = titles;
        classArray = childVCs;
        colorArray = colors;
        _parentViewController = controllers;
...
- (void)createPagerView:(NSArray *)titles WithVCs:(NSArray *)childVCs WithColors:(NSArray *)colors {
    ...
    if (titles.count > 0 && childVCs.count > 0) {
        ...
        if (classArray.count > 0 && myArray.count > 0) {
            ...
            if (class) {
                UIViewController *ctrl = [[UIStoryboard storyboardWithName:@"TB" bundle:nil] instantiateViewControllerWithIdentifier:className];
                if (_type != 0) {
                    VC_Base *vc = (VC_Base *)ctrl;
                    vc.parameters = @{kPageType: @(_type)};
                }
                ...
                [_parentViewController addChildViewController:ctrl];
                viewAlloc[0] = YES;
            }
        }
    }else {
            ...
            if (page == i && i <= classArray.count - 1) {
                ...
                if (class && viewAlloc[i] == NO) {
                    ...
                    if (_type != 0) {
                        VC_Base *vc = (VC_Base *)ctrl;
                        vc.parameters = @{kPageType: @(_type)};
                        if ([vc isKindOfClass:[VC_Tech_Order class]]) {
                            VC_Tech_Order *vc_tech_order = (VC_Tech_Order *)vc;
                            vc_tech_order.type = _type;
                        }
                    }
                    [pagerView.scrollView addSubview:ctrl.view];
                    [_parentViewController addChildViewController:ctrl];
