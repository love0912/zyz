...
- (void)drawRect:(CGRect)rect
{
    ...
    // 画金额
    NSString *money;
    if (self.moneyTag == nil || [self.moneyTag isEqualToString:@""]) {
        money = [NSString stringWithFormat:@"金额: %@",[self setMoneyComma:self.money isAndDecimal:YES]];
    } else {
        money = [NSString stringWithFormat:@"%@: %@",self.moneyTag, [self setMoneyComma:self.money isAndDecimal:YES]];
    }
    ...
    moneyAttr[NSForegroundColorAttributeName] = [CommonUtil zyzOrangeColor];//LPColor(209, 47, 27);
