 ...
 *  金额前面显示的名称
 */
@property (nonatomic, copy) NSString *moneyTag;

/** 完成的回调block */
@property (nonatomic, copy) void (^finish) (NSString *passWord);

/** 点击取消按钮回调*/
@property (nonatomic, copy) void (^cancel) (BOOL cancel);
@end
...
- (void)setupInputNumberView
{
    ...
    inputView.moneyTag = self.moneyTag;
...
/** 快速创建6位数字的密码, 自定义金额前面的名称 带Block回调密码 带自定义数字键盘 只能输入6位数字密码 */
+ (instancetype)tradeViewNumberKeyboardWithMoney:(NSString *)money moneyTag:(NSString *)moneyTag password:(void (^) (NSString *password))password {
    LPTradeView *view = [[LPTradeView alloc] init];
...
+ (instancetype)tradeViewNumberKeyboardWithMoney:(NSString *)money moneyTag:(NSString *)moneyTag password:(void (^) (NSString *password))password cancel:(void (^)(BOOL cancel))cancel {
    LPTradeView *view = [[LPTradeView alloc] init];
