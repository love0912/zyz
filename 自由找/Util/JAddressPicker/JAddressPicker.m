...
#define JAddressPickerHeight SCREEN_HEIGHT * 0.35

@interface JAddressPicker ()
...
- (void)internalConfig {
    ...
    button.frame = CGRectMake(10, 0, 46, 43.5);
    ...
    button.frame = CGRectMake(self.frame.size.width - 42, 0, 46, 43.5);
    button.backgroundColor = [UIColor clearColor];
    button.titleLabel.font = [UIFont systemFontOfSize:16];
    [button setTitleColor:[UIColor colorWithRed:0/255.0 green:122/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
...
- (void)showPickerWithProvinceName:(NSString *)provinceName cityName:(NSString *)cityName countyName:(NSString *)countyName {
    _firstAry = kJAddressManager.provinceDicAry;
    
    if (provinceName.length > 0) {
        [_firstAry enumerateObjectsUsingBlock:^(NSDictionary *provinceDic, NSUInteger idx, BOOL * _Nonnull stop) {
            if ([provinceDic[@"Value"] isEqualToString:provinceName]) {
                _firstCurrentIndex = idx+1;
                _secondAry = provinceDic[@"SubRegions"];
            }
        }];
    } else {
    ...
    [_secondAry enumerateObjectsUsingBlock:^(NSDictionary *cityDic, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([cityDic[@"Value"] isEqualToString:cityName]) {
            _secondCurrentIndex = idx+1;
            NSArray *subArray = cityDic[@"SubRegions"];
            if (subArray.count > 0) {
                _thirdAry = cityDic[@"SubRegions"];
            }
        }
    }];
    
    [_thirdAry enumerateObjectsUsingBlock:^(NSDictionary *countyDic, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([countyDic[@"Value"] isEqualToString:countyName]) {
...
- (void)show {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.pickView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - JAddressPickerHeight, self.frame.size.width, JAddressPickerHeight);
...
- (void)hide {
    _backView.alpha = 0;
    [UIView animateWithDuration:0.5 animations:^{
        self.pickView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height+44, self.frame.size.width, JAddressPickerHeight);
...
- (UIPickerView*)pickView {
    if (!_pickView) {
        _pickView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height + 44, self.frame.size.width, JAddressPickerHeight)];
