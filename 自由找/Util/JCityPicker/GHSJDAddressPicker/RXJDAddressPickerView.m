...
- (instancetype)initWithSelectArea:(NSString *)selectArea {
    CGRect frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
    self = [self initWithFrame:frame];
    if (self) {
        NSArray *areas = [selectArea componentsSeparatedByString:@","];
        [areas enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *areaName = obj;
            if (idx == 0) {
                [_provinceArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSDictionary *tmpDic = obj;
                    if ([tmpDic[@"Value"] isEqualToString:areaName]) {
                        _provinceLine = idx;
                        _cityArray = [NSMutableArray arrayWithArray:tmpDic[@"SubRegions"]];
                        *stop = YES;
                        _selfSelectCount = 1;
                        [self createButtonsWithTitle:areaName andIndex:1];
                    }
                }];
            } else if (idx == 1) {
                [_cityArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    ...
                    if ([tmpDic[@"Value"] isEqualToString:areaName]) {
                        _cityLine = idx;
                        _areaArray = [NSMutableArray arrayWithArray:tmpDic[@"SubRegions"]];
                        _selfSelectCount = 2;
                        [self createButtonsWithTitle:areaName andIndex:2];
                        *stop = YES;
                    }
                }];
            } else {
                [_areaArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                    NSDictionary *tmpDic = obj;
                    if ([tmpDic[@"Value"] isEqualToString:areaName]) {
                        _areaLine = idx;
                        _selfSelectCount = 3;
                        [self createButtonsWithTitle:areaName andIndex:3];
                        *stop = YES;
                    }
                }];
            }
...
- (void)configUI {
    ...
    _provinceLine     = 10; _cityLine    = 0;  _areaLine    = 0;
    _provineceVisible = 10; _cityVisible = 0;  _areaVisible = 0;
    _selfSelectCount  = 0;
    
    _buttonsLasterLeft = 21;
    _selectLineViewLeft = 21;
    
    _provinceArray = [RXGetAddress getLocalAreaArray];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == _provinceTableView) {
        ...
        titleString = [[_provinceArray objectAtIndex:row] objectForKey:@"Value"];
        ...
        titleString = [[_cityArray objectAtIndex:row] objectForKey:@"Value"];
        ...
        titleString = [[_areaArray objectAtIndex:row] objectForKey:@"Value"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == _provinceTableView) {
       ...
       name = [[_provinceArray objectAtIndex:row] objectForKey:@"Value"];
        //城市数组
        _cityArray = _provinceArray[_provinceLine][@"SubRegions"];
        ...
        name = [[_cityArray objectAtIndex:row] objectForKey:@"Value"];
        
        //区的
        _areaArray = _cityArray[_cityLine][@"SubRegions"];
        ...
        name = [[_areaArray objectAtIndex:row] objectForKey:@"Value"];
        [_areaTableView reloadData];
...
+ (void)showWithSelectArea:(NSString *)selectAreaName completion:(JDPickerShowOrHidden)completion {
    RXJDAddressPickerView *jPicker = [[RXJDAddressPickerView alloc] initWithSelectArea:selectAreaName];
    jPicker.completion = completion;
    [[UIApplication sharedApplication].keyWindow addSubview:jPicker];
    [jPicker show];
}
...
- (void)completionAddress {
    ...
    NSString * provinceName = _provinceArray[_provinceLine][@"Value"];
    NSString * cityName = _cityArray[_cityLine][@"Value"];
    NSString * areaName = _areaArray[_areaLine][@"Value"];
    
//    NSString * provinceCode = _provinceArray[_provinceLine][@"code"];
    NSString * cityCode = _cityArray[_cityLine][@"Key"];
    NSString * areaCode = _areaArray[_areaLine][@"Key"];
